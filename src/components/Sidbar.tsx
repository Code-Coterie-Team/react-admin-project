// import React from 'react';
// import { Link } from 'react-router-dom';

const Sidbar = () => {
    return (
        <div className="sidbar p-4 border-r-[1px] border-solid border-grayb col-span-2 h-full w-full">

            <button className="icon-menu hover:bg-bgHover hover:rounded-full">
                <svg width="40" height="40" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                    <path fill-rule="evenodd" clip-rule="evenodd" d="M6.25 8C6.25 7.58579 6.58579 7.25 7 7.25H17C17.4142 7.25 17.75 7.58579 17.75 8C17.75 8.41421 17.4142 8.75 17 8.75H7C6.58579 8.75 6.25 8.41421 6.25 8ZM6.25 12C6.25 11.5858 6.58579 11.25 7 11.25H17C17.4142 11.25 17.75 11.5858 17.75 12C17.75 12.4142 17.4142 12.75 17 12.75H7C6.58579 12.75 6.25 12.4142 6.25 12ZM6.25 16C6.25 15.5858 6.58579 15.25 7 15.25H17C17.4142 15.25 17.75 15.5858 17.75 16C17.75 16.4142 17.4142 16.75 17 16.75H7C6.58579 16.75 6.25 16.4142 6.25 16Z" fill="#FFFFFF" />
                </svg>
            </button>

            <nav className="pt-8 flex flex-col">
              
                <div className="overView flex p-4 mb-2 hover:bg-bgHover rounded-lg">
                    <svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                        <path fill-rule="evenodd" clip-rule="evenodd" d="M12 3C13.1046 3 14 3.89543 14 5L14 19C14 20.1046 13.1046 21 12 21C10.8954 21 10 20.1046 10 19L10 5C10 3.89543 10.8954 3 12 3ZM4 12C5.10457 12 6 12.8954 6 14V19C6 20.1046 5.10457 21 4 21C2.89543 21 2 20.1046 2 19V14C2 12.8954 2.89543 12 4 12ZM22 10C22 8.89543 21.1046 8 20 8C18.8954 8 18 8.89543 18 10V19C18 20.1046 18.8954 21 20 21C21.1046 21 22 20.1046 22 19V10Z" fill="#5b5ee7" />
                    </svg>
                    <span className="pl-3 font-medium ">
                        Overview
                    </span>
                </div>
             
                <div className="products flex  p-4 hover:bg-bgHover rounded-lg">
                    <svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                        <path fill-rule="evenodd" clip-rule="evenodd" d="M9.5 19.75C9.08579 19.75 8.75 20.0858 8.75 20.5C8.75 20.9142 9.08579 21.25 9.5 21.25C9.91421 21.25 10.25 20.9142 10.25 20.5C10.25 20.0858 9.91421 19.75 9.5 19.75ZM7.25 20.5C7.25 19.2574 8.25736 18.25 9.5 18.25C10.7426 18.25 11.75 19.2574 11.75 20.5C11.75 21.7426 10.7426 22.75 9.5 22.75C8.25736 22.75 7.25 21.7426 7.25 20.5Z" fill="#8c5df7" />
                        <path fill-rule="evenodd" clip-rule="evenodd" d="M18.5 19.75C18.0858 19.75 17.75 20.0858 17.75 20.5C17.75 20.9142 18.0858 21.25 18.5 21.25C18.9142 21.25 19.25 20.9142 19.25 20.5C19.25 20.0858 18.9142 19.75 18.5 19.75ZM16.25 20.5C16.25 19.2574 17.2574 18.25 18.5 18.25C19.7426 18.25 20.75 19.2574 20.75 20.5C20.75 21.7426 19.7426 22.75 18.5 22.75C17.2574 22.75 16.25 21.7426 16.25 20.5Z" fill="#8c5df7" />
                        <path fill-rule="evenodd" clip-rule="evenodd" d="M1.25 2C1.25 1.58579 1.58579 1.25 2 1.25H4C5.25878 1.25 6.32002 2.09575 6.64648 3.25H18C20.6234 3.25 22.75 5.37665 22.75 8V13C22.75 15.6234 20.6234 17.75 18 17.75H10C7.37665 17.75 5.25 15.6234 5.25 13V4C5.25 3.30964 4.69036 2.75 4 2.75H2C1.58579 2.75 1.25 2.41421 1.25 2ZM6.75 8.75V13C6.75 14.7949 8.20507 16.25 10 16.25H18C19.7949 16.25 21.25 14.7949 21.25 13V8.75H6.75ZM21.163 7.25H6.75V4.75H18C19.5368 4.75 20.8245 5.81665 21.163 7.25Z" fill="#8c5df7" />
                    </svg>
                    <span className="pl-3 font-medium ">
                        Products
                    </span>
                </div>
             
               
                <div className="users mb-2 flex  p-4 hover:bg-bgHover rounded-lg">
                    <svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                        <path fill-rule="evenodd" clip-rule="evenodd" d="M5.38566 15.6959C4.19864 16.2895 3.75 16.9706 3.75 17.5C3.75 18.0294 4.19864 18.7105 5.38566 19.3041C6.52782 19.8751 8.15819 20.25 10 20.25C11.8418 20.25 13.4722 19.8751 14.6143 19.3041C15.8014 18.7105 16.25 18.0294 16.25 17.5C16.25 16.9706 15.8014 16.2895 14.6143 15.6959C13.4722 15.1249 11.8418 14.75 10 14.75C8.15819 14.75 6.52782 15.1249 5.38566 15.6959ZM4.71484 14.3543C6.10618 13.6586 7.97582 13.25 10 13.25C12.0242 13.25 13.8938 13.6586 15.2852 14.3543C16.6316 15.0275 17.75 16.0964 17.75 17.5C17.75 18.9036 16.6316 19.9725 15.2852 20.6457C13.8938 21.3414 12.0242 21.75 10 21.75C7.97582 21.75 6.10618 21.3414 4.71484 20.6457C3.36836 19.9725 2.25 18.9036 2.25 17.5C2.25 16.0964 3.36836 15.0275 4.71484 14.3543Z" fill="#eb4798" />
                        <path fill-rule="evenodd" clip-rule="evenodd" d="M10 3.75C8.20507 3.75 6.75 5.20507 6.75 7C6.75 8.79493 8.20507 10.25 10 10.25C11.7949 10.25 13.25 8.79493 13.25 7C13.25 5.20507 11.7949 3.75 10 3.75ZM5.25 7C5.25 4.37665 7.37665 2.25 10 2.25C12.6234 2.25 14.75 4.37665 14.75 7C14.75 9.62335 12.6234 11.75 10 11.75C7.37665 11.75 5.25 9.62335 5.25 7Z" fill="#eb4798" />
                        <path fill-rule="evenodd" clip-rule="evenodd" d="M15.0448 10.2496C14.7228 10.7485 14.3288 11.1965 13.8774 11.5791C14.2319 11.6901 14.609 11.75 15.0001 11.75C17.0712 11.75 18.7501 10.0711 18.7501 8C18.7501 6.04422 17.2529 4.43814 15.3421 4.26538C15.6083 4.78435 15.8011 5.34717 15.9068 5.94015C16.6979 6.28887 17.2501 7.07994 17.2501 8C17.2501 9.2277 16.2668 10.2257 15.0448 10.2496Z" fill="#eb4798" />
                        <path fill-rule="evenodd" clip-rule="evenodd" d="M18.9997 17.5563C18.9896 18.1704 18.8148 18.7605 18.5009 19.3108C18.8693 19.2094 19.2144 19.092 19.5312 18.96C20.1284 18.7112 20.6606 18.3958 21.055 18.0073C21.452 17.6162 21.7501 17.1064 21.7501 16.4999C21.7501 15.8935 21.452 15.3836 21.055 14.9925C20.6606 14.604 20.1284 14.2887 19.5312 14.0399C18.5086 13.6138 17.1907 13.3394 15.7495 13.2683C16.7517 13.7774 17.5702 14.4169 18.1352 15.1442C18.4329 15.2274 18.7072 15.3215 18.9543 15.4245C19.443 15.6281 19.7894 15.8514 20.0023 16.0611C20.2125 16.2682 20.2501 16.416 20.2501 16.4999C20.2501 16.5838 20.2125 16.7317 20.0023 16.9388C19.7961 17.1419 19.4645 17.3578 18.9997 17.5563Z" fill="#eb4798" />
                    </svg>


                    <span className="pl-3 font-medium ">
                        Users
                    </span>
                </div>
                <div className="sales mb-2 flex  p-4 hover:bg-bgHover rounded-lg">
                    <svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                        <path
                            fill-rule="evenodd"
                            clip-rule="evenodd"
                            d="M12 5.75C10.4812 5.75 9.25 6.98122 9.25 8.5C9.25 10.0188 10.4812 11.25 12 11.25C12.4142 11.25 12.75 11.5858 12.75 12C12.75 12.4142 12.4142 12.75 12 12.75C9.65279 12.75 7.75 10.8472 7.75 8.5C7.75 6.15279 9.65279 4.25 12 4.25C14.3472 4.25 16.25 6.15279 16.25 8.5C16.25 8.91421 15.9142 9.25 15.5 9.25C15.0858 9.25 14.75 8.91421 14.75 8.5C14.75 6.98122 13.5188 5.75 12 5.75Z"
                            fill="#13ba82" />
                        <path
                            fill-rule="evenodd"
                            clip-rule="evenodd"
                            d="M11.25 12C11.25 11.5858 11.5858 11.25 12 11.25C14.3472 11.25 16.25 13.1528 16.25 15.5C16.25 17.8472 14.3472 19.75 12 19.75C9.65279 19.75 7.75 17.8472 7.75 15.5C7.75 15.0858 8.08579 14.75 8.5 14.75C8.91421 14.75 9.25 15.0858 9.25 15.5C9.25 17.0188 10.4812 18.25 12 18.25C13.5188 18.25 14.75 17.0188 14.75 15.5C14.75 13.9812 13.5188 12.75 12 12.75C11.5858 12.75 11.25 12.4142 11.25 12Z"
                            fill="#13ba82" />
                        <path
                            fill-rule="evenodd"
                            clip-rule="evenodd"
                            d="M12 2.25C12.4142 2.25 12.75 2.58579 12.75 3V21C12.75 21.4142 12.4142 21.75 12 21.75C11.5858 21.75 11.25 21.4142 11.25 21V3C11.25 2.58579 11.5858 2.25 12 2.25Z"
                            fill="#13ba82" />
                    </svg>

                    <span className="pl-3 font-medium ">
                        Sales
                    </span>
                </div>
                <div className="orders mb-2 flex  p-4 hover:bg-bgHover rounded-lg">
                    <svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                        <path
                            fill-rule="evenodd"
                            clip-rule="evenodd"
                            d="M9.5 19.75C9.08579 19.75 8.75 20.0858 8.75 20.5C8.75 20.9142 9.08579 21.25 9.5 21.25C9.91421 21.25 10.25 20.9142 10.25 20.5C10.25 20.0858 9.91421 19.75 9.5 19.75ZM7.25 20.5C7.25 19.2574 8.25736 18.25 9.5 18.25C10.7426 18.25 11.75 19.2574 11.75 20.5C11.75 21.7426 10.7426 22.75 9.5 22.75C8.25736 22.75 7.25 21.7426 7.25 20.5Z"
                            fill="#f69f0c" />
                        <path
                            fill-rule="evenodd"
                            clip-rule="evenodd"
                            d="M18.5 19.75C18.0858 19.75 17.75 20.0858 17.75 20.5C17.75 20.9142 18.0858 21.25 18.5 21.25C18.9142 21.25 19.25 20.9142 19.25 20.5C19.25 20.0858 18.9142 19.75 18.5 19.75ZM16.25 20.5C16.25 19.2574 17.2574 18.25 18.5 18.25C19.7426 18.25 20.75 19.2574 20.75 20.5C20.75 21.7426 19.7426 22.75 18.5 22.75C17.2574 22.75 16.25 21.7426 16.25 20.5Z"
                            fill="#f69f0c" />
                        <path
                            fill-rule="evenodd"
                            clip-rule="evenodd"
                            d="M1.25 2C1.25 1.58579 1.58579 1.25 2 1.25H4C5.25878 1.25 6.32002 2.09575 6.64648 3.25H18C20.6234 3.25 22.75 5.37665 22.75 8V13C22.75 15.6234 20.6234 17.75 18 17.75H10C7.37665 17.75 5.25 15.6234 5.25 13V4C5.25 3.30964 4.69036 2.75 4 2.75H2C1.58579 2.75 1.25 2.41421 1.25 2ZM6.75 8.75V13C6.75 14.7949 8.20507 16.25 10 16.25H18C19.7949 16.25 21.25 14.7949 21.25 13V8.75H6.75ZM21.163 7.25H6.75V4.75H18C19.5368 4.75 20.8245 5.81665 21.163 7.25Z"
                            fill="#f69f0c" />
                    </svg>
                    <span className="pl-3 font-medium ">
                        Orders
                    </span>
                </div>
                <div className="analytics mb-2 flex  p-4 hover:bg-bgHover rounded-lg">
                    <svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                        <path
                            fill-rule="evenodd"
                            clip-rule="evenodd"
                            d="M20 4.75C19.3096 4.75 18.75 5.30964 18.75 6C18.75 6.4331 18.9696 6.81521 19.3071 7.04062C19.5051 7.17289 19.7424 7.25 20 7.25C20.6904 7.25 21.25 6.69036 21.25 6C21.25 5.30964 20.6904 4.75 20 4.75ZM17.25 6C17.25 4.48122 18.4812 3.25 20 3.25C21.5188 3.25 22.75 4.48122 22.75 6C22.75 7.51878 21.5188 8.75 20 8.75C19.7061 8.75 19.4223 8.70367 19.1561 8.61789L14.0916 16.2147C14.5016 16.6947 14.75 17.3182 14.75 18C14.75 19.5188 13.5188 20.75 12 20.75C10.4812 20.75 9.25 19.5188 9.25 18C9.25 17.6523 9.31481 17.3191 9.43296 17.0122L5.66663 14.1875C5.20454 14.5399 4.62675 14.75 4 14.75C2.48122 14.75 1.25 13.5188 1.25 12C1.25 10.4812 2.48122 9.25 4 9.25C5.51878 9.25 6.75 10.4812 6.75 12C6.75 12.3477 6.68519 12.6809 6.56704 12.9878L10.3334 15.8125C10.7955 15.4601 11.3733 15.25 12 15.25C12.2939 15.25 12.5777 15.2963 12.8439 15.3821L17.9084 7.78533C17.4984 7.30535 17.25 6.68176 17.25 6ZM4 10.75C3.30964 10.75 2.75 11.3096 2.75 12C2.75 12.6904 3.30964 13.25 4 13.25C4.40845 13.25 4.77111 13.0549 5.00044 12.7496C5.15733 12.5408 5.25 12.2823 5.25 12C5.25 11.3096 4.69036 10.75 4 10.75ZM12 16.75C11.5915 16.75 11.2289 16.9451 10.9996 17.2504C10.8427 17.4592 10.75 17.7177 10.75 18C10.75 18.6904 11.3096 19.25 12 19.25C12.6904 19.25 13.25 18.6904 13.25 18C13.25 17.5669 13.0304 17.1848 12.6929 16.9594C12.4949 16.8271 12.2576 16.75 12 16.75Z"
                            fill="#3d83f7" />
                    </svg>
                    <span className="pl-3 font-medium ">
                        Analytics
                    </span>
                </div>
                <div className="setting flex p-4 hover:bg-bgHover rounded-lg">
                    <svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                        <path
                            fill-rule="evenodd"
                            clip-rule="evenodd"
                            d="M8.70452 5.6C8.70452 4.26116 9.84226 3.25 11.1515 3.25H12.8485C14.1577 3.25 15.2955 4.26116 15.2955 5.6C15.2955 6.0422 15.7686 6.32334 16.157 6.11192L16.2472 6.06282C17.3925 5.43933 18.8763 5.79336 19.5635 6.91552L20.412 8.30117C21.1163 9.45148 20.6786 10.901 19.5098 11.5372C19.1431 11.7368 19.1431 12.2632 19.5098 12.4628C20.6786 13.099 21.1163 14.5485 20.412 15.6988L19.5635 17.0845C18.8763 18.2066 17.3925 18.5607 16.2472 17.9372L16.157 17.8881C15.7686 17.6767 15.2955 17.9578 15.2955 18.4C15.2955 19.7388 14.1577 20.75 12.8485 20.75H11.1515C9.84226 20.75 8.70452 19.7388 8.70452 18.4C8.70452 17.9578 8.2314 17.6767 7.84301 17.8881L7.7528 17.9372C6.60743 18.5607 5.12365 18.2067 4.4365 17.0845L3.58801 15.6988C2.88363 14.5485 3.32141 13.099 4.49018 12.4628C4.85686 12.2632 4.85686 11.7368 4.49018 11.5372C3.32141 10.901 2.88363 9.45147 3.58801 8.30115L4.4365 6.91551C5.12365 5.79335 6.60743 5.43932 7.75281 6.06281L7.84301 6.11191C8.2314 6.32333 8.70452 6.0422 8.70452 5.6ZM11.1515 4.75C10.5863 4.75 10.2045 5.17153 10.2045 5.6C10.2045 7.18023 8.51376 8.18488 7.12585 7.42936L7.03564 7.38026C6.55771 7.12009 5.96579 7.29046 5.71572 7.69884L4.86723 9.08448C4.63441 9.4647 4.75281 9.9723 5.20734 10.2197C6.6178 10.9875 6.6178 13.0125 5.20734 13.7803C4.75281 14.0277 4.63441 14.5353 4.86723 14.9155L5.71572 16.3012C5.96578 16.7095 6.55771 16.8799 7.03564 16.6197L7.12585 16.5706C8.51376 15.8151 10.2045 16.8198 10.2045 18.4C10.2045 18.8285 10.5863 19.25 11.1515 19.25H12.8485C13.4137 19.25 13.7955 18.8285 13.7955 18.4C13.7955 16.8198 15.4862 15.8151 16.8741 16.5706L16.9643 16.6197C17.4423 16.8799 18.0342 16.7095 18.2843 16.3012L19.1327 14.9155C19.3656 14.5353 19.2472 14.0277 18.7926 13.7803C17.3822 13.0125 17.3822 10.9875 18.7926 10.2197C19.2472 9.97231 19.3656 9.46471 19.1327 9.08449L18.2843 7.69885C18.0342 7.29048 17.4423 7.12011 16.9643 7.38027L16.8741 7.42937C15.4862 8.18489 13.7955 7.18023 13.7955 5.6C13.7955 5.17153 13.4137 4.75 12.8485 4.75H11.1515Z"
                            fill="#6fe8b8" />
                        <path
                            fill-rule="evenodd"
                            clip-rule="evenodd"
                            d="M12 9.75C10.7574 9.75 9.75 10.7574 9.75 12C9.75 13.2426 10.7574 14.25 12 14.25C13.2426 14.25 14.25 13.2426 14.25 12C14.25 10.7574 13.2426 9.75 12 9.75ZM8.25 12C8.25 9.92893 9.92893 8.25 12 8.25C14.0711 8.25 15.75 9.92893 15.75 12C15.75 14.0711 14.0711 15.75 12 15.75C9.92893 15.75 8.25 14.0711 8.25 12Z"
                            fill="#6fe8b8" />
                    </svg>
                    <span className="pl-3 font-medium ">
                        Setting
                    </span>
                </div>
            </nav>
        </div>
    )
}

export default Sidbar
